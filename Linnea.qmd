...
data2018 %>%
  ...
  mutate(HoldTimeMins = as.numeric(lubridate::ymd_hms(LabAnalysisDatetime) - lubridate::ymd_hms(SampleDatetime))*60*24) %>%
   ...
   mutate(HoldCriteria = case_when("Constituents" == "Cond" ~ 40320,
                                  "Constituents" == "Turb" ~ 1440,
                                  "Constituents" == "ColA" ~ 2880,
                                  "Constituents" == "TSS" ~ 10080,
                                  "Constituents" == "EcoliIQT" ~ 360,
                                  "Constituents" == "BOD" ~ 360,
                                  "Constituents" == "TotP" ~ 40320,
                                  "Constituents" == "Alk" ~ 20160,
                                  "Constituents" == "ColT" ~ 2880,
                                  "Constituents" == "ColiformIQT" ~ 360,
                                  TRUE ~ HoldTimeMins
                                  )) %>%
  
  select(SiteCode, SampleDatetime, ResultValue, QCType, Constituents, ResultFlag_ID, IncubationLength, LabAnalysisDatetime, HoldTime, HoldTimeMins, HoldCriteria) %>%
  distinct() %>% 
  gt() %>%
