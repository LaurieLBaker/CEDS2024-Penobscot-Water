...
HoldTimeMins <- data2018 %>%
  mutate(HoldTimeMins = (difftime(LabAnalysisDatetime, SampleDatetime, units = "mins"), na.rm = TRUE))

HoldViol <- function(HoldTimeMins) {
  if(Constituents == Cond) {HoldTimeMins-40320}
  if(Constituents == Turb) {HoldTimeMins-1440}
  if(Consituents == )
}
