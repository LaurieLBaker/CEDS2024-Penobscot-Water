...
data2018 %>%
  ...
  mutate(HoldTimeMins = as.numeric(lubridate::ymd_hms(LabAnalysisDatetime) - lubridate::ymd_hms(SampleDatetime))*60*24) %>%
   ...
   mutate(HoldViol = case_when(Constituents == "Cond" ~ HoldTimeMins - 40320,
  ...
  select(SiteCode, SampleDatetime, ResultValue, QCType, Constituents, ResultFlag_ID, IncubationLength, LabAnalysisDatetime, HoldTime, HoldViol) %>%
  distinct() %>% 
  gt() %>%
