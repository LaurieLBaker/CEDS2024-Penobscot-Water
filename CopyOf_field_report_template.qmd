...
data2018 %>% 
  ...
  rename("Run Code" = "RunCode",
         "Run Date" = "RunDate",
         "Primary Collector" = "PrimCollector") %>%
  distinct() %>% 
  gt() 

data2018 %>% 
  filter(SiteVisitID == "JanS5-2018925102747") %>% 
  select(WaterBody, SiteCode, SiteVisitStartTime, SiteDepth) %>%
...
data2018 %>%
  ...
  select(SampleName, Agency_ID, AgencySampleNumber, CollMethod:ResultFlag_ID) %>%
  distinct() %>%
  gt()
```
