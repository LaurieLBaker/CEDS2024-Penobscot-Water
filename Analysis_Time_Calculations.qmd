...
data2018 %>% 
  separate_longer_delim(cols = Constituents, delim = ";") %>%
  mutate(BacIncMins = as.numeric(ymd_hms(ProcessDateTime) - ymd_hms(LabAnalysisDatetime)) * 60 * 24) %>% 
  mutate(BacInc = seconds_to_period(as.numeric(BacIncMins) * 60))
  
  
  mutate(BacIncMins = as.numeric(lubridate::ymd_hms(ProcessDateTime) - lubridate::ymd_hms(LabAnalysisDatetime)) * 60 * 24) %>%
  mutate(BacInc = seconds_to_period(as.numeric(BacIncMins) * 60))
```
